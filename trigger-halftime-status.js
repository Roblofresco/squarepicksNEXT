...
try {
  ...
  async function triggerHalftimeStatus() {
    try {
      const gameId = process.argv[2] || '401772943';
      
      console.log(`Triggering halftime status for game: ${gameId}\n`);
      ...
      // First, set status to something else (to ensure transition)
      const currentStatus = gameData.status || 'in_progress';
      
      if (currentStatus !== 'halftime') {
        console.log(`🔄 Step 1: Setting status to a non-halftime value first...`);
      ...
      // Now change to halftime to trigger the function
      console.log(`🔄 Step 2: Changing status to "halftime"...`);
      await gameRef.update({
        status: 'halftime',
      ...
      console.log(`   Quarter: ${updatedGameData.quarter || 'Unknown'}`);
      ...
      console.log('   The onGameUpdatedAssignWinners function should have processed the status change.');
      
    } catch (error) {
      console.error('❌ Error triggering halftime status:', error);
      throw error;
    } finally {
      process.exit(0);
    }
